...
func search(t *testing.T, expression string, data any) (any, error) {
	t.Helper()
...
func TestCanSupportStructWithSliceAll(t *testing.T) {
	...
	assert.Nil(err)
	assert.Equal([]any{"f1", "correct"}, result)
}
...
func TestCanSupportFlattenNestedSlice(t *testing.T) {
	...
	assert.Nil(err)
	assert.Equal([]any{"f1a", "f1b", "f2a", "f2b"}, result)
}
